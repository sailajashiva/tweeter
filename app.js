...
ON tweet.user_id=follwer_user_id
...
app.post('/login/', async (request, response) => {
  ...
  const getUserWuery = `SELECT * FROM user WHERE username='${username}';`
  const userDBDetails = await db.get(getUserQuery)
...
app.get('/user/followers/', authentication, async (request, response) => {
  const {username, userId} = request
  const getFollowersQuery = `SELECT DISTNICT name FROM follower
     INNER JOIN user ON user.user_id = follower.following_user_id
     WHERE follower_user_id= '${userId}';`
...
app.get("/tweets/:tweetId/", authentication, tweetAccessVerification, async (request, response) =>{
  ...
  const usersArray=likedUsers.app((eachUser)=> eachUser.username);
...
app.get("/tweets/:tweetId/replies", authentication, tweetAccessVerification, async (request, response) =>{
  ...
  FROM user INNER JOIN like ON user.user_id= reply.user_id
  WHERE tweet_id='${tweetId}'; `;
...
app.get("/users/tweets/", authentication, async (request, response) =>{
  const { userId } = request;
  const getTweetQuery = `SELECT tweet,
      COUNT(DISTNIC like_id) AS likes,
      COUNT(DISTNIC reply_id) AS replies,
      date_time AS dateTime
